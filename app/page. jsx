...
const STAGES = ["Encounter", "PIDS", "Prayer", "Pending"];
const ACTIVITIES = ["BBQ", "Beach", "Hiking", "Pickleball"];
...
export default function FamilyGroupTracker() {
  ...
  const exportRoster = () => {
    const head = ["Full Name", "Age", "Phone #", "Group", "Under", "Stage", ...ACTIVITIES];
    const body = students.map((s) => [
      ...
      parentOf(s.leader) || "",
      s.stage,
      ...ACTIVITIES.map((a) => (s.activities?.includes(a) ? 1 : 0)),
    ]);
  ...
  const addPeople = async () => {
    const fresh = names
      ...
      .map((n) => n.trim())
      .filter(Boolean)
      .map((n) => ({ name: n, group_id: idOf[addLeader], stage: "Pending" }));
  ...
  const toggleActivity = (s, a) =>
    editStudent(s.id, {
      activities: s.activities?.includes(a)
        ? s.activities.filter((x) => x !== a)
        : [...(s.activities || []), a],
    });
    ...
    <aside className="hidden w-60 shrink-0 flex-col border-r border-neutral-200 bg-white p-3 md:flex">
      <div className="flex items-center gap-3 px-2 py-3">
        ...
        <div className="leading-tight">
          <div className="text-[15px] font-semibold text-neutral-900">Family Groups</div>
      ...
      <main className="min-w-0 flex-1">
        <div className="sticky top-0 z-10 border-b border-neutral-200 bg-white px-4 py-3 md:hidden">
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-sm font-semibold">Family Groups</span>
          </div>
                    ...
                    >
                      ...
                      <ul>
                        {list.map((s) => (
                          <li
                            key={s.id}
                            className="flex items-center justify-between gap-3 border-t border-neutral-100 px-4 py-2.5 first:border-t-0"
                          >
                              ...
                              <span className="flex gap-1">
                                {ACTIVITIES.map((a) => (
                                  <Chip
                                    key={a}
                                    title={a}
                                    on={s.activities?.includes(a)}
                                    onClick={() => toggleActivity(s, a)}
                                  >
                                    {a === "Pickleball" ? "Pkl" : a.slice(0, 3)}
                                  </Chip>
                                ))}
                              </span>
                              <button
