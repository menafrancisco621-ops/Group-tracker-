...
export default function FamilyGroupTracker() {
  ...
  const [codeKind, setCodeKind] = useState("leader");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const out = {};
      if (codeKind === "person") {
        const g = codeGroup || allGroups[0];
        for (const x of rosterOf(g)) out[x.id] = await QRCode.toDataURL(personLink(x.id), opts);
      } else {
        const url = codeKind === "checkin" ? checkinLink : linkFor;
        for (const g of allGroups) out[g] = await QRCode.toDataURL(url(g), opts);
      }
  ...
  }, [view, allGroups, codeKind, codeGroup, students]);
  ...
  const blank = { name: "", age: "", phone: "", stage: "Not on path", track: "" };
  ...
  if (scanned) {
    ...
    const said = all.filter((x) => marks[x.id]?.c).length;
    return (
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="font-mono text-[12px] tabular-nums text-neutral-500">
                  {here} of {all.length} present · {said} confirmed
                </div>
              </div>
              ...
              {list.map((x) => (
                ...
                >
                  ...
                  <span className="flex shrink-0 gap-1.5">
                    <Chip on={marks[x.id]?.c} onClick={() => mark(x.id, "c")}>
                      Confirmed
                    </Chip>
          ...
          {view === "codes" && (
            <>
              ...
              <p className="mb-5 text-[13px] text-neutral-500 print:hidden">
                {codeKind === "leader" &&
                  "For the leader's own phone. Opens their group's full attendance list, this week, ready to mark."}
                {codeKind === "checkin" &&
                  "For the table or the door. Members scan it, tap their own face, and they're marked present."}
                {codeKind === "person" &&
                  "One card per member. Scanning or tapping it marks that person present instantly — no list, no tapping."}
              </p>

              {codeKind === "person" && (
                <select
                  value={codeGroup || allGroups[0] || ""}
                  onChange={(e) => setCodeGroup(e.target.value)}
                  className="mb-5 h-10 w-full rounded-lg border border-neutral-300 bg-white px-3 text-sm print:hidden sm:w-64"
                >
                  {allGroups.map((g) => (
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {codeKind === "person"
                  ? rosterOf(codeGroup || allGroups[0] || "").map((x) => (
                      <div
                        key={x.id}
                      ...
                      >
                        {codes[x.id] ? (
                          ...
                          <div className="mx-auto flex h-36 w-36 items-center justify-center text-[12px] text-neutral-300">
                            making code…
                          </div>
                        )}
                        <div className="mt-2 truncate text-[15px] font-semibold">{x.name}</div>
                        <div className="text-[12px] text-neutral-500">{x.leader}</div>
                        <div className="mt-3 flex justify-center gap-2 print:hidden">
                          <button
                            onClick={() => copyLink(x.id, personLink(x.id))}
                          ...
                          <button
                            onClick={() => writeTag(x.name, personLink(x.id))}
                            className="rounded-lg border border-neutral-300 px-2.5 py-1 text-[12px] hover:border-neutral-900"
                          >
                            Write NFC
                          </button>
                  ...
                  : allGroups.map((g) => {
                      const p = parentOf(g);
                      const url = codeKind === "checkin" ? checkinLink(g) : linkFor(g);
                      return (
                        <div
                          key={g}
                          className="break-inside-avoid rounded-xl border border-neutral-200 bg-white p-4 text-center"
