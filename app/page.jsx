            ...
            <>
              ...
              {empty ? (
                ...
                <>
                  ...
                  <div className="rounded-xl border border-neutral-200 bg-white p-4">
                    <div className="flex items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1.5 text-[13px] text-neutral-500">
                          of {totals.total} present · {totals.pct}%
                        </div>
                      </div>
                      {(() => {
                        const prev = LEADERS.reduce(
                          (t, l) => t + presentIn(l, shiftWeeks(week, -1)),
                          0
                        );
                        const dv = totals.present - prev;
                        return (
                          <div className="text-right">
                            <div
                              ...
                              style={{
                                color:
                                  dv > 0 ? "#0ca30c" : dv < 0 ? "#d03b3b" : "#a3a3a3",
                              }}
                            >
                              {dv > 0 ? `▲ ${dv}` : dv < 0 ? `▼ ${-dv}` : "—"}
                            </div>
                            <div className="text-[12px] text-neutral-400">vs last week</div>
                          </div>
                        );
                      })()}
                    </div>
                    ...
                    {(() => {
                      const series = lastWeeks.map((k) =>
                        LEADERS.reduce((t, l) => t + presentIn(l, k), 0)
                      );
                      const max = Math.max(1, ...series);
                      const pts = series
                        .map(
                          (n, i) =>
                            `${4 + (i * 312) / (series.length - 1 || 1)},${
                              48 - (n / max) * 40
                            }`
                        )
                        .join(" ");
                      ...
                      return (
                        <svg
                          ...
                          role="img"
                          aria-label="Attendance over the last eight weeks"
                        >
                          <polyline
                            points={pts}
                            fill="none"
                  ...
                  {/* one square per leader */}
                  <div className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-4">
                    {rows.map((r) => {
                      const hasSubs = r.subs.length > 1;
                      const isOpen = !!open[r.leader];
                      const hue = hueOf(r.leader);
                      const dv = r.present - presentIn(r.leader, shiftWeeks(week, -1));
                      return (
                        <div
                          key={r.leader}
                          ...
                          >
                            <div className="flex items-start justify-between">
                              ...
                              <span
                                ...
                                style={{
                                  color:
                                    dv > 0 ? "#0ca30c" : dv < 0 ? "#d03b3b" : "#d4d4d4",
                                }}
                              >
                                {dv > 0 ? `▲${dv}` : dv < 0 ? `▼${-dv}` : "—"}
                              </span>
                            ...
                            <div className="mt-3 h-1.5 w-full rounded-full bg-neutral-100">
                              <div
                                ...
                                style={{ width: `${r.pct}%`, background: hue }}
                              />
                            </div>
                          </button>
